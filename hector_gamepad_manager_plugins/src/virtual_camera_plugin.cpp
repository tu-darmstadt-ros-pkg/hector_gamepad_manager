...
void VirtualCameraPlugin::initialize( const rclcpp::Node::SharedPtr &node )
{
  ...
  pan_speed_param_sub_ = hector::createReconfigurableParameter(
      node, plugin_namespace + ".pan_speed", std::ref( pan_speed_ ),
  ...
  tilt_speed_param_sub_ = hector::createReconfigurableParameter(
      node, plugin_namespace + ".tilt_speed", std::ref( tilt_speed_ ),
  ...
  back_camera_topic_name_sub_ = hector::createReconfigurableParameter(
      node, plugin_namespace + ".back_camera_topic_name", std::ref( back_camera_topic_name_ ), "ROS topic name of the back camera." );

  front_camera_topic_name_sub_ = hector::createReconfigurableParameter(
      node, plugin_namespace + ".front_camera_topic_name", std::ref( front_camera_topic_name_ ), "ROS topic name of the front camera." );
  ...
  invert_y_axis_param_sub_ = hector::createReconfigurableParameter(
      node, plugin_namespace + ".invert_y_axis", std::ref( invert_y_axis_ ), "Whether to invert the Y axis of the camera control." );

  loadCameras();
  RCLCPP_INFO( node_->get_logger(), "Initialized VirtualCameraPlugin with back camera topic '%s' and front camera topic '%s'", back_camera_topic_name_.c_str(), front_camera_topic_name_.c_str() );}
...
void VirtualCameraPlugin::loadCameras()
{
  ...
  front_transform_publisher_ = node_->create_publisher<geometry_msgs::msg::Transform>( front_topic_name, 1 );
  back_transform_publisher_ = node_->create_publisher<geometry_msgs::msg::Transform>( back_topic_name, 1 );
}
...
void VirtualCameraPlugin::update()
{
  ...
  double pan = reverse_ ? back_transform_[2] : front_transform_[2];
  double tilt = reverse_ ? back_transform_[1] : front_transform_[1]; 
  Eigen::Isometry3d orientation = Eigen::Isometry3d(Eigen::AngleAxisd(pan, Eigen::Vector3d::UnitZ()) *
