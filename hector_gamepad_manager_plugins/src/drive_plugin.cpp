...
void DrivePlugin::initialize( const rclcpp::Node::SharedPtr &node )
{
  ...
  blackboard_->set( "inverted_steering", false );
}
...
void DrivePlugin::handlePress( const std::string &function )
{
  if ( function == "fast" ) {
    ...
    bool &inverted = blackboard_->at<bool>( "inverted_steering" ); // inverted_steering exists
    inverted = !inverted;
    msg.data = inverted;
...
void DrivePlugin::update()
{
  ...
  bool inverted_steering = blackboard_->value_or<bool>( "inverted_steering", false );
  const double steering_inv = inverted_steering ? -1.0 : 1.0;
